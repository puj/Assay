...
- **The whole thread, not just what's on screen.** Both sites render only a window of a long conversation, so an export can only ever see what is rendered. Assay reads the thread when you press ⭳ — and goes on reading it for as long as the message picker is open, so scrolling back with the picker up fills in the rest of the conversation and you watch the count climb as it does. What it has read is kept locally, per conversation, and the picker tells you per message how much was captured and whether it came from the page or from memory. Messages are identified by their text, so nothing is lost when a site changes its markup, and identical messages stay distinct. Nothing is ever dropped automatically — a message you can't see may be one the page hasn't loaded — and ↻ in the picker forgets this conversation and reads the page again when a thread has been edited elsewhere.
- **It does nothing when you aren't asking it to.** Close the picker and Assay stops reading the page entirely: no observers, no polling, nothing on scroll. Typing, scrolling and a reply streaming in cost nothing, because nothing of ours is attached to them — which is the only way a highlight stays under your finger on a phone.
...
Both sites load a long conversation in pieces: older messages appear only as you scroll to them, and some sections stay collapsed until opened. An export can only ever see what is rendered. Pressing ⭳ captures everything rendered at that moment, which for a conversation you have been having in front of you is all of it. For an older thread, leave the picker open and scroll back: for as long as it is open Assay keeps reading the page, and the count in the header climbs as it picks messages up. What it reads is kept locally, in the site's own storage, so a thread you have walked through once exports whole afterwards. Each row shows how many characters were captured and whether it is on screen or remembered; tap that to read exactly what is held.

**Does it slow the page down?**
No, because with the picker closed it is not watching the page at all — no observers, no polling, nothing on scroll. Reading the thread costs a layout, so it happens only when you ask: opening the tray, pressing ⭳, or scrolling with the picker open.
