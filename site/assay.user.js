...
(function () {
  ...
  function textOf(el) {
    ...
    // A message the page has skipped rendering — content-visibility, a list
    // that draws only what is near the screen — has its words in the document
    // and none of them in innerText. Take what is in the document then.
    var tc = (el.textContent || '').replace(/\u00a0/g, ' ').trim();
    if (tc.length > 40 && t.length * 2 < tc.length) t = tc.replace(/[ \t]+/g, ' ').replace(/\s*\n\s*/g, '\n');
  ...
  // attributes still has these. A label's turn is the highest wrapper that
  // holds this label and no other, when that wrapper holds more than the
  // label. When it does not — the label sits in a wrapper of its own and the
  // message follows as a sibling — the turn is the widest of the siblings
  // between it and the next label. A label with no message is not a turn.
  var SAID_RE = /^(you|chatgpt|assistant|[a-z0-9][a-z0-9 ._-]{0,30}) said:?$/i;
  var LABEL_PREFIX = /^(you|chatgpt|assistant|[a-z0-9][a-z0-9 ._-]{0,30}) said:?\s*/i;
  function unlabel(t) { return (t || '').replace(LABEL_PREFIX, ''); }
  ...
  function turnOfLabel(h, prev, next) {
    var hl = normText(h.textContent).length, A = h, p;
    while ((p = A.parentElement) && p !== document.body && p.tagName !== 'MAIN' &&
           !(prev && p.contains(prev)) && !(next && p.contains(next))) A = p;
    if (normText(A.textContent).length - hl >= BLOCK_MIN) return A;
    var best = null, bestLen = 0;
    for (var sib = A.nextElementSibling; sib && !(next && sib.contains(next)); sib = sib.nextElementSibling) {
      if (host.contains(sib) || (sib.closest && sib.closest(EDITABLE_SEL))) continue;
      var len = normText(sib.textContent).length;
      if (len >= BLOCK_MIN && len > bestLen) { best = sib; bestLen = len; }
    }
    return best;
  }
  function saidTurns() {
    ...
    for (i = 0; i < hs.length; i++) {
      var h = hs[i], turn = turnOfLabel(h, hs[i - 1], hs[i + 1]);
  ...
  // What the site itself calls this message, if it says. chatgpt.com has
  // called it data-message-id, then data-turn-id, and now writes it on a
  // search-unit wrapper inside the turn as data-chatgpt-search-message-ids.
  function turnIdOf(el) {
  ...
  function liveTurns() {
    ...
    return best.els.map(function (el) {
      ...
      if (!seen || seen.tl !== tl || seen.kids !== kids) {
        var norm = unlabel(normText(el.innerText || ''));
  ...
  // ChatGPT numbers its turns — <article data-testid="conversation-turn-N"> —
  // and that number is the one fact about order that no amount of scrolling,
  // loading or re-rendering can disturb. Where it exists it decides the order
  // of the export outright; everything else is a fallback for pages without it.
  function turnNumberOf(el) {
    try {
      var a = el.closest && el.closest('[data-testid^="conversation-turn-"]');
      if (!a) return -1;
      var n = parseInt(a.getAttribute('data-testid').replace('conversation-turn-', ''), 10);
      return isNaN(n) ? -1 : n;
    } catch (e) { return -1; }
  }
  ...
  function scrollsY(n) {
    ...
    return (o === 'auto' || o === 'scroll' || o === 'overlay') && n.scrollHeight > n.clientHeight + 40;
  }
  // Set a scroll position now — not smoothly, whatever the page's own
  ...
  function describeEl(n) {
    if (!n) return 'none';
    if (n === document.scrollingElement || n === document.documentElement) return 'document';
    var cls = ((n.className && n.className.indexOf ? n.className : '') + '').split(/\s+/).filter(Boolean).slice(0, 3).join('.');
    return n.tagName.toLowerCase() + (n.id ? '#' + n.id : '') + (cls ? '.' + cls : '') +
      ' ' + n.scrollHeight + '/' + n.clientHeight;
  }
  // Every element on the page that could be the thread's scroller, largest
  // first: the ancestors of a message, the document, and anything else that
  // scrolls and is more than a strip. Told apart by whether they move.
  function scrollerCandidates(el) {
    var out = [], n = el;
    while (n && n !== document.body && n !== document.documentElement) {
      if (scrollsY(n) && out.indexOf(n) < 0) out.push(n);
      n = n.parentElement;
    }
    var de = document.scrollingElement || document.documentElement;
    if (de && de.scrollHeight > de.clientHeight + 40 && out.indexOf(de) < 0) out.push(de);
    var all = document.querySelectorAll('div,main,section');
    for (var i = 0; i < all.length; i++) {
      var c = all[i];
      if (host.contains(c) || c.clientHeight < 200 || out.indexOf(c) >= 0) continue;
  ...
  // helpers above stay for the diagnosis, which is where they earn their keep.
  ...
  // Viewers are not folds. "Expand table" on chatgpt.com opens the table in a
  // dialog over the thread — the rows were in the message all along — and the
  // dialog locks the thread's scroll, so a walk that opened one stopped there.
  var EXPAND_NEVER = /\b(send|submit|delete|remove|retry|regenerate|stop|share|copy|edit|branch|fork|report|sign|log ?out|upgrade|table|image|chart|code|canvas|diagram|artifact|preview|full ?screen|zoom|enlarge|download|open in)\b/i;
  function dialogsOpen() {
    var n = 0, ds = document.querySelectorAll('[role="dialog"],dialog[open]');
    for (var i = 0; i < ds.length; i++) if (!host.contains(ds[i])) n++;
    return n;
  }
  // A click that opened a dialog opened something that was not a fold; close
  // it, or the walk reads a locked thread.
  function closeDialog() {
    var ds = document.querySelectorAll('[role="dialog"],dialog[open]'), i, j;
    for (i = 0; i < ds.length; i++) {
      if (host.contains(ds[i])) continue;
      var btns = ds[i].querySelectorAll('button,[role="button"]');
      for (j = 0; j < btns.length; j++) {
        var l = ((btns[j].getAttribute('aria-label') || '') + ' ' + (btns[j].textContent || '')).trim();
        if (/^(close|dismiss|done|×|✕|✖)$/i.test(l) || /\b(close|dismiss)\b/i.test(l)) { try { btns[j].click(); } catch (e) {} break; }
      }
    }
    try {
  ...
  var dialogsClosed = 0;
  function expandOnce() {
    var opened = 0, turns = liveTurns(), i, j, before = dialogsOpen();
    for (i = 0; i < turns.length; i++) {
      ...
      for (j = 0; j < btns.length; j++) {
        ...
        if (dialogsOpen() > before) { closeDialog(); dialogsClosed++; opened--; }
      }
    }
    return opened;
  }
  ...
  function windowTurns() {
    var list = inReadingOrder(liveTurns());
    return list.map(function (t, i) {
      return {
        role: t.role, el: t.el, id: t.id, turnNo: t.turnNo, head: t.head, len: t.len, y: t.y,
        b: serializeBlocks(t.el), key: turnKey(t),
        // A message with no id of its own is told apart from an identical one
        // elsewhere by what comes before it: the second "Yes." in a thread has
        // a different neighbour from the first. The first message in a window
        // has no neighbour in view, which absorb allows for.
        base: turnKey(t),
        prev: i > 0 ? turnKey(list[i - 1]) : null
      };
    });
  }
  // Putting the windows together.
  //
  // One slot per message, full stop. Earlier versions allowed a key to appear
  // twice if it appeared twice within one window, to keep two identical
  // messages apart — and a page that momentarily holds two copies of a row
  // while it re-renders turned that into a permanent duplicate. Identical
  // messages are told apart by their neighbour now (see windowTurns), so the
  // rule here can be the simple one.
  function absorb(acc, win, at) {
    at.byQual = at.byQual || {};
    at.byBase = at.byBase || {};
    function fuller(idx, t) {
      var held = acc[idx];
      // The same message, read again with more of it showing.
      if ((t.b || []).join('\n').length > (held.b || []).join('\n').length) {
        t.prev = t.prev || held.prev;
        acc[idx] = t;
      }
    }
    function add(t) {
      var idx = acc.length;
      acc.push(t);
      (at.byBase[t.base] = at.byBase[t.base] || []).push(idx);
      if (t.prev !== null) at.byQual[t.base + '<' + t.prev] = idx;
      return idx;
    }
    for (var i = 0; i < win.length; i++) {
      var t = win[i];
      if (t.id) {
        // An id settles it. Nothing about neighbours applies.
        if (at[t.id] === undefined) at[t.id] = add(t); else fuller(at[t.id], t);
        continue;
      }
      var known = at.byBase[t.base] || [];
      if (t.prev !== null) {
        var q = t.base + '<' + t.prev;
        if (at.byQual[q] !== undefined) { fuller(at.byQual[q], t); continue; }
        // Held already, but first met at the top of a window where its
        // neighbour was out of view: this is that message, now with context.
        var orphan = -1;
        for (var k = 0; k < known.length; k++) if (acc[known[k]].prev === null) { orphan = known[k]; }
        if (orphan >= 0) { acc[orphan].prev = t.prev; at.byQual[q] = orphan; fuller(orphan, t); continue; }
        add(t);
      } else {
        // Top of a window, neighbour out of view. If a message that reads
        // like this is already held, it is that one — the most recent of them,
        // since the walk goes downward and this sits just after what was read
        // last. Only a message never seen before starts a new entry.
        if (known.length) { fuller(known[known.length - 1], t); continue; }
        add(t);
      }
    }
    return acc;
  ...
  // The order of the export. Where the page numbers its turns, that number is
  // the order and nothing about how the walk went can change it. Where it does
  // not, the order is the order the capture pass met them in — and the capture
  // pass only counts if the document held still for the whole of it.
  function finish(acc) {
    var numbered = acc.length > 0;
    for (var i = 0; i < acc.length; i++) if (!(acc[i].turnNo >= 0)) { numbered = false; break; }
    if (numbered) {
      acc = acc.map(function (t, i) { return { t: t, i: i }; })
        .sort(function (a, b) { return (a.t.turnNo - b.t.turnNo) || (a.i - b.i); })
        .map(function (w) { return w.t; });
    }
    // Belt and braces: an id can appear once, whatever else happened.
    var seen = {}, out = [];
  ...
  var HARVEST_MS = 150000;       // a ceiling, so a strange page cannot hang here
  var QUIET_MS = 2500;           // how long the top has to hold still to count as the top
  var harvestStop = false;
  var lastWalk = null;           // what the last walk saw, for the diagnosis
  function harvest(onProgress) {
    return new Promise(function (resolve) {
      ...
      // What the top of the document looks like: which turns are built while
      // the scroll sits at zero. Not how tall the page is, which changes as
      // images load and folds open, and not the first turn alone: chatgpt.com
      // keeps the first turn in the document at every scroll position, so by
      // that measure the top had "settled" before a single older message had
      // arrived, and the walk missed a third of a long thread.
      function snapshot() {
        return inReadingOrder(liveTurns()).map(function (t) {
          return t.turnNo >= 0 ? t.turnNo : turnKey(t);
        }).join(',');
      }
      function label(t) { return t.turnNo >= 0 ? t.turnNo : (t.id || t.head.slice(0, 12)); }
      function finishWith(acc) {
        walk.stopped = harvestStop;
        walk.dialogs = dialogsClosed;
        var out = finish(acc);
        // The trace records what the export is, not what the walk held before
      ...
      // every scroll position — as chatgpt.com does — the number of the final
      // turn is known from the first window, and the walk is done the moment
      // every number from the first to it is held. The rest of the scroll would
      // only read the same pinned tail eighty more times.
      function complete(acc) {
        ...
        for (var k = lo; k <= hi; k++) if (!seen[k]) return false;
        return hi - lo + 1 === acc.length;
      }

      // 1. The top. Not "unchanged twice" — a batch of older messages is a
      // network round trip, and on a phone that is easily longer than two
      // polls — but "nothing has changed for a good while". The scroll is
      // nudged off zero and back between polls, since some pages load the next
      // batch only on a fresh crossing of the top.
      var quiet = 0, last = '';
      function up(nudge) {
        ...
        setTimeout(function () {
          ...
          setTimeout(function () {
            ...
            if (quiet >= QUIET_MS) return expandDown(0);
            up(!nudge);
          }, 400);
        }, 100);
      }
      ...
      // 3. Read, top to bottom, over a document that is holding still. If the
      ...
      function capture(attempt) {
        ...
        function at_(pos) {
          ...
          setTimeout(function () {
            ...
            // the screen is a different set of turns every time.
  ...
  $('diagBtn').addEventListener('click', function () {
    ...
    try {
      ...
      if (f.walk) {
        ...
        lines.push('  final order: ' + JSON.stringify(f.walk.final));
      }
      ...
      if (f.census) {
        ...
        (f.census.labelTurns || []).forEach(function (lt) {
          lines.push('  label: ' + lt.label);
          lines.push('    turn: ' + lt.turn);
        });
        ...
        if (f.census.tallAncestors) lines.push('  tall ancestors: ' + f.census.tallAncestors.join(' | '));
        if (f.census.ua) lines.push('  browser: ' + f.census.ua);
      }
  ...
  $('fullYes').addEventListener('click', function () {
    ...
    harvest(function (msg, n) {
      ...
      // Say what the walk can vouch for. On a page that numbers its turns the
      // numbers should run without a gap; a gap is a message the page never
      // built, and that is worth knowing before the file is written.
      var numbered = turns.every(function (t) { return t.turnNo >= 0; }), gaps = 0;
      if (numbered) {
        for (var i = 1; i < turns.length; i++) if (turns[i].turnNo > turns[i - 1].turnNo + 1) gaps++;
      }
      var vouch = numbered ? (gaps ? ', ' + gaps + ' gap' + (gaps === 1 ? '' : 's') + ' the page never loaded' : ', in order, none repeated') : '';
      toast(harvestStop
        ? 'Stopped — ' + turns.length + ' messages'
        : (gained > 0 ? 'Found ' + gained + ' more — ' + turns.length + ' messages' + vouch
                      : 'That was already all of it — ' + turns.length + ' messages' + vouch), 3200);
    }, function () {
      harvesting = false;
      setScan(false);
    ...
    _find: function () {
      ...
      var named = out.strategies.some(function (st) { return st.n > 0; });
      {
        ...
        ['article', '[data-turn]', '[data-turn-id]', '[data-message-id]', '[data-message-author-role]',
         '[class*="markdown"]', '[class*="prose"]', 'main', '[role="presentation"]', '[data-testid]',
         '[data-turn-key]', '[data-virtualized-turn-content]', '[data-chatgpt-search-message-ids]', '[data-conversation-role]'].forEach(function (q) {
        ...
        try {
          ...
          // The first label of each side, where the finder put its turn, and
          // how much text that turn holds — the one thing a paste has to say
          // when the label finder is the one that answered.
          var hs = saidHeadings(), shown = {};
          out.census.labelTurns = [];
          for (var li = 0; li < hs.length && out.census.labelTurns.length < 2; li++) {
            var side = /^you\b/i.test(normText(hs[li].textContent)) ? 'you' : 'them';
            if (shown[side]) continue;
            shown[side] = true;
            var tn = turnOfLabel(hs[li], hs[li - 1], hs[li + 1]);
            out.census.labelTurns.push({ label: pathOf(hs[li]), turn: tn ? pathOf(tn) : 'none' });
          }
          var md = document.querySelector('main [class*="markdown"], [class*="markdown"]');
          if (md) out.census.markdownPath = pathOf(md);
          // What the walk would scroll, and whether each candidate moves.
          var f0 = live[0] && live[0].el;
          if (f0) {
            var cands = scrollerCandidates(f0);
            out.census.scrollers = cands.slice(0, 6).map(function (c) {
              return describeEl(c) + (c.contains(f0) ? ' holds-thread' : '') + (moves(c) ? ' moves' : ' stuck');
            });
            // Every ancestor of the first message with a scroll range, whatever
            // its overflow says: the element the page really scrolls may be
            // one that hides its overflow and scrolls itself.
            var chain = [], up = f0;
            while (up && up !== document.body && chain.length < 12) {
              if (up.scrollHeight > up.clientHeight + 40) {
                var ucs = getComputedStyle(up);
                chain.push(describeEl(up) + ' ' + ucs.overflowY + (moves(up) ? ' moves' : ' stuck'));
              }
              up = up.parentElement;
            }
            out.census.tallAncestors = chain;
