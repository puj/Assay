...
- **Export is what the page is showing.** Both sites render only a window of a long conversation, so ⭳ reads the thread at the moment you press it: scroll back and open what's collapsed first and the file has more of it. Messages are found by several markup strategies, so identical messages stay distinct and a site redesign doesn't empty the export.
- **Nothing of Assay runs while you use the chat.** No observers, no polling, nothing on scroll, nothing remembered between exports. Scrolling, typing and a reply streaming in cost exactly zero — measured as zero frames requested, not as a fast number — and highlights are painted by the browser into the text itself rather than as boxes held over it, so they move with the words instead of chasing them. On a phone that is the difference between a highlight that stays under your finger and one that doesn't.
