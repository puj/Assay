(function () {
  ...
  // then every window is read from the same place and nothing on the screen
  ...
  var lastScrollers = null;      // what the last walk had to choose from, for the diagnosis
  function scrollerFor(el) {
    var cands = scrollerCandidates(el), pick = null, i;
    lastScrollers = cands.map(function (c) { return describeEl(c) + (c.contains(el) ? ' holds-thread' : ''); });
    for (i = 0; i < cands.length && !pick; i++) if (moves(cands[i])) pick = cands[i];
    if (!pick && cands.length) pick = cands[0];
    lastScrollers.picked = pick ? describeEl(pick) + (moves(pick) ? ' (moves)' : ' (does not move)') : 'none';
    return pick;
  }
