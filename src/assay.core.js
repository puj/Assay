(function () {
  ...
  function harvest(onProgress) {
    return new Promise(function (resolve) {
      ...
      // What the top of the document looks like: how many turns there are,
      // which is first, how tall it all is. If any of it moves, the page is
      // still loading and nothing read so far can be trusted.
      function snapshot() {
        var l = liveTurns();
        return l.length + '|' + (l[0] ? turnKey(l[0]) : '') + '|' + sc.scrollHeight;
      }
      function finishWith(acc) {
        walk.stopped = harvestStop;
        walk.final = acc.map(function (t) { return t.turnNo >= 0 ? t.turnNo : (t.id || t.head.slice(0, 12)); });
      ...
      function capture(attempt) {
        var acc = [], at = {}, before = snapshot();
        walk.passes++;
        function at_(pos) {
          sc.scrollTop = pos;
          setTimeout(function () {
            var win = windowTurns();
            walk.windows.push(win.map(function (t) { return t.turnNo >= 0 ? t.turnNo : (t.id || t.head.slice(0, 12)); }));
